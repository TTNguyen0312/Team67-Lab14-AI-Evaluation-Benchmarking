...
async def run_benchmark_with_results(agent_version: str):
    ...
    # Aggregate agent token usage across all test cases
    agg_agent: Dict[str, Any] = {"total_input_tokens": 0, "total_output_tokens": 0, "total_tokens": 0, "total_cost_usd": 0.0, "by_model": {}}
    for r in results:
        tu = r.get("agent_token_usage", {})
        model = tu.get("model", "unknown")
        inp = tu.get("input_tokens", 0)
        out = tu.get("output_tokens", 0)
        cost = tu.get("cost_usd", 0.0)
        agg_agent["total_input_tokens"] += inp
        agg_agent["total_output_tokens"] += out
        agg_agent["total_tokens"] += tu.get("total_tokens", 0)
        agg_agent["total_cost_usd"] += cost
        if model not in agg_agent["by_model"]:
            agg_agent["by_model"][model] = {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0, "cost_usd": 0.0}
        agg_agent["by_model"][model]["input_tokens"] += inp
        agg_agent["by_model"][model]["output_tokens"] += out
        agg_agent["by_model"][model]["total_tokens"] += tu.get("total_tokens", 0)
        agg_agent["by_model"][model]["cost_usd"] += cost
    agg_agent["total_cost_usd"] = round(agg_agent["total_cost_usd"], 6)

    # Aggregate judge token usage across all test cases
    agg_judge: Dict[str, Any] = {"total_input_tokens": 0, "total_output_tokens": 0, "total_tokens": 0, "total_cost_usd": 0.0, "by_model": {}}
    for r in results:
        tu = r.get("judge_token_usage", {})
        agg_judge["total_input_tokens"] += tu.get("total_input_tokens", 0)
        agg_judge["total_output_tokens"] += tu.get("total_output_tokens", 0)
        agg_judge["total_tokens"] += tu.get("total_tokens", 0)
        agg_judge["total_cost_usd"] += tu.get("total_cost_usd", 0.0)
        for model, usage in tu.get("by_model", {}).items():
            if model not in agg_judge["by_model"]:
                agg_judge["by_model"][model] = {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0, "cost_usd": 0.0}
            agg_judge["by_model"][model]["input_tokens"] += usage.get("input_tokens", 0)
            agg_judge["by_model"][model]["output_tokens"] += usage.get("output_tokens", 0)
            agg_judge["by_model"][model]["total_tokens"] += usage.get("total_tokens", 0)
            agg_judge["by_model"][model]["cost_usd"] += usage.get("cost_usd", 0.0)
    agg_judge["total_cost_usd"] = round(agg_judge["total_cost_usd"], 6)

    agg_tokens = {
        "agent": agg_agent,
        "judge": agg_judge,
        "total_cost_usd": round(agg_agent["total_cost_usd"] + agg_judge["total_cost_usd"], 6),
    }
